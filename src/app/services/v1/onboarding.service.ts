...
export class OnboardingService {
  ...
  recoveryPassword = async (email: string) => {
    ...
    // Logic
    // eslint-disable-next-line no-shadow
    const uid = userExists.dataValues.id;
    ...
    console.log('uid', uid, userRecord);
  ...
  changePassword = async (IUserRecoveryDTO: IUser.RecoveryDTO) => {
    ...
    const codeExist: any = await passwordResetRepository.singleCondition({
      code
    });
    ...
    // Generate Password
    // eslint-disable-next-line no-shadow
    const uid = codeExist.dataValues.uid;
    const hashedPassword = IUserRecoveryDTO.password
      ? sha1(IUserRecoveryDTO.password).toString()
      : null;

    console.log('eee', uid, hashedPassword);

    const userRecord = await BoAdmin.update(
      {
        password: hashedPassword,
        status: 'active'
      },
